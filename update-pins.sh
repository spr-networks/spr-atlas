...
# lookups (registry manifest inspect, go.dev, GitHub tags). Review with `git diff`.
...
# RIPE Atlas software probe: latest production release tag. Upstream versioning:
# tags divisible by 10 are production releases (5100, 5110, 5120, ...); anything
# else is testing/devel. Pin the tag's peeled (full) commit hash.
echo "Resolving RIPE Atlas release tag..." >&2
ATLAS_TAGS=$(git ls-remote --tags "$ATLAS_GIT")
ATLAS_VERSION=$(echo "$ATLAS_TAGS" | awk -F'refs/tags/' '{print $2}' \
  | grep -E '^[0-9]+0(\^\{\})?$' | tr -d '^{}' | sort -n | uniq | tail -1)
[ -n "$ATLAS_VERSION" ] || { echo "could not resolve an Atlas production tag" >&2; exit 1; }
# Prefer the peeled hash (annotated tags), fall back to the direct ref.
ATLAS_COMMIT=$(echo "$ATLAS_TAGS" | awk -v t="refs/tags/${ATLAS_VERSION}^{}" '$2==t{print $1}')
if [ -z "$ATLAS_COMMIT" ]; then
  ATLAS_COMMIT=$(echo "$ATLAS_TAGS" | awk -v t="refs/tags/${ATLAS_VERSION}" '$2==t{print $1}')
fi
[ -n "$ATLAS_COMMIT" ] || { echo "could not resolve commit for tag ${ATLAS_VERSION}" >&2; exit 1; }
echo "RIPE Atlas: ${ATLAS_VERSION} @ ${ATLAS_COMMIT}" >&2
...
# RIPE Atlas software probe: latest production release tag (divisible by 10)
# pinned by full commit hash. https://github.com/RIPE-NCC/ripe-atlas-software-probe
